...
- Formaty kadru mają jedno źródło: `src/lib/formats.ts` (`FRAME_FORMATS`). Stamtąd prompt serwera bierze pola i limity wierszy, studio posta bierze kształt wypełnienia, a `frameToBeats` — rozkład na takty. Studio ma **cztery** formaty: Cytat, Protokół, Koszt, Kolaż. Diagram, „Napis w scenie" jako wybór, siatka życia i audyt doby zostały **skasowane** (użycie raz i na nic; „napis w scenie" zostaje tylko jako układ z analizy linku, bo 1:1 odtwarza cudzy kadr) — nie wracaj do nich bez nowego uzasadnienia.
...
- Tła z AI w studiu posta nie ma (`POST /api/ai/generate-background` zostało poza ścieżką). Darmowy tier nie ma modelu obrazów, a „tło z banku scen" w jego miejsce to ten sam zakazany bank treści.
