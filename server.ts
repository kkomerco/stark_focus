...
const VOID_MATRIX = {
  pains: [
    "lenistwo", "wymówki", "komfort", "prokrastynacja",
    "porównywanie się", "tania dopamina", "brak planu", "strach przed oceną"
  ],
  truths: [
    "nikt nie przyjdzie", "czas ucieka - klepsydra", "jesteś sam",
    "nikt nie patrzy", "komfort cię zabija", "jutro to kłamstwo", "dyscyplina to kara za wczoraj"
  ],
  formats: [
  ...
  hooks: [
    "To cię zniszczy", "Przestań kłamać", "Masz 24h",
    "Nikt ci tego nie powie", "Klepsydra nie czeka", "SF RULE #"
  ],
  actions: [
    "wstań", "odtnij ich", "zamknij mordę i rób", "zasada 1%", "protokół 04:30"
  ],
};
...
app.get('/api/proxy-image', async (req, res) => {
  ...
  try {
    // Blokujemy przekierowania (redirect: 'manual'), aby zapobiec obchodzeniu SSRF
    ...
    if (r.status >= 300 && r.status < 400) {
      return res.status(400).json({ error: 'Przekierowania URL są zablokowane ze względów bezpieczeństwa' });
    }

    if (!r.ok) return res.status(r.status).json({ error: 'Nie udało się pobrać obrazu' });

    const ct = r.headers.get('content-type') || '';
    if (!ct.startsWith('image/')) return res.status(400).json({ error: 'Zasób nie jest obrazem' });
...
app.post('/api/void/replicate', async (req, res) => {
  const { url } = req.body;
  if (!url || !isSafeUrl(url)) return res.status(400).json({ error: 'Nieprawidłowy lub zablokowany URL' });
  res.json({
    url,
    detected: { photos: 4, hasQuoteOnBlack: true, changingBg: false, timing: "0.8s / 1.1s / 0.8s / 2.2s" },
    template: {
      type: "VOID_4_PHASE",
      slides: [{ placeholder: "EMPTY BED" }, { placeholder: "EMPTY WALLET" }, { placeholder: "MIRROR" }, { placeholder: "FUTURE SELF" }]
    }
  });
});
...
// Obsługa SPA (Vite fallback dla odświeżania podstron)
app.get('*', (req, res) => {
  res.sendFile(path.join(__dirname, 'dist', 'index.html'));
});

app.listen(PORT, () => console.log(`🚀 SF VOID / Visionary Media Lab running on http://localhost:${PORT}`));
