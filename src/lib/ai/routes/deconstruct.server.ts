...
import { getOfflineStarkVariants } from "../offline-content";
...
export function registerDeconstructRoutes(app: MiniApp): void {
  ...
  app.post("/api/ai/deconstruct-viral", async (req, res) => {
    ...
    if (!isSafeUrl(cleanUrl)) {
      return res.status(400).json({ error: "Nieprawidłowy lub zablokowany URL (ochrona SSRF)" });
    }
    ...
    if (!ai) {
      return res.json({
        ...
        starkVariants: getOfflineStarkVariants(dynamicSeed, 2),
      });
    }
